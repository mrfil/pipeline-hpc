#!/bin/bash
# perform QSIprep routine without a T1w structural reference - experimental
...
projDir=${tmpdir}/${project}
	...
	rm ${projDir}/bids/derivatives/${subject}/${sesname}/test.txt	
		...
		SINGULARITY_CACHEDIR=${scachedir} SINGULARITY_TMPDIR=${stmpdir} singularity run --cleanenv --bind ${IMAGEDIR}:/imgdir,${stmpdir}:/paulscratch,${projDir}:/data ${IMAGEDIR}/qsiprep-v0.14.3.sif --fs-license-file /imgdir/license.txt /data/bids /data/bids/derivatives --recon_input /data/bids/derivatives/qsiprep --recon_spec mrtrix_multishell_msmt --output-resolution 2.0 -w /paulscratch participant --participant-label ${subject} --skip_bids_validation --dwi_only --skip-t1-based-spatial-normalization
		NOW=$(date +"%m-%d-%Y-%T")
